...
class Lattice():
    ...
    def polynomial_elliptical_hole_taper(self, number_of_tapered_holes, hx , hy , w,
                                         a_center ,  
                                         a_mirror):
    ...
    def load_data(self, filename = "sweep_data.hdf5"):    # CHANGE CHANGE CHANGE
        '''
...
def polynomial_fit(data, w, hy, hx, gamma_mirror, degree = 10, a_mirror = 0.347 ):            
    ...
    gamma_NZ = gamma_arr[ gamma_arr > 0]                           # non zero gammas
    
    gamma_interest = gamma_NZ[ : np.where( (gamma_NZ == gamma_mirror))[0][0]]   # for each gamma, there are 2 values of a, hence, selected only one
